...
def android_write(og_cur, cur):
    ...
    create_index_sql = f"""
        CREATE UNIQUE INDEX idx_android ON android(file, source);
    """
    ...
    sql = f"""
        INSERT OR IGNORE INTO android (file, source, data) VALUES (?,?,?)
        """
...
def execute_query(cursor: sqlite3.Connection, qcomps: QueryComponents) -> list[Any]:
    ...
    # orders by match type if reading was provided
